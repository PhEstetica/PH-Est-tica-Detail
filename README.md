...
## V13.4 — Configurações por abas + backup/restauração
- Admin → Configurações agora foi dividido em 5 abas: Identidade e contato, Mensagens do WhatsApp, Imagens da página inicial, Logo do cabeçalho e Backups.
- Cada salvamento retorna para a aba correta, evitando a tela longa/confusa.
- Aba Backups ganhou duas ações destacadas: baixar backup e restaurar backup.
- Restauração aceita `.db` e `.zip`; antes de substituir o banco ativo, o sistema cria uma cópia de segurança automática.
- O status do armazenamento mostra claramente quando o Persistent Disk `/var/data` está ativo.
- Layout das abas e backup foi adaptado para PC, tablet e celular.

## V13.5 — Preços “A partir de”, avaliação e novos serviços adicionais

Esta versão mantém tudo da V13.4 e melhora a forma como os preços são apresentados ao cliente.

### Preços mais seguros para a PH
- Serviços principais podem ser exibidos como **Preço fixo** ou **A partir de**.
- Os serviços atuais passam a usar **A partir de** por padrão, sem alterar o valor-base salvo.
- Adicionais podem ser **Preço fixo**, **A partir de** ou **Sob avaliação**.
- O cliente recebe avisos claros durante o agendamento de que o valor pode mudar conforme o estado interno e externo do veículo, nível de sujeira, barro, graxa, areia, pelos, manchas e tempo necessário.
- O resumo final mostra **Estimativa inicial** e identifica itens sob avaliação.
- Em sujeira moderada/pesada, uma Lavagem Simples recebe aviso de que pode exigir ajuste ou um serviço mais indicado.

### Novos adicionais para carros
Valores cadastrados são valores iniciais sugeridos e continuam editáveis pelo Admin:
- Remoção de chuva ácida dos vidros — a partir de R$ 40
- Hidrorepelência dos vidros — a partir de R$ 30
- Descontaminação da pintura — a partir de R$ 40
- Proteção da pintura / selante — a partir de R$ 30
- Revitalização de plásticos externos — a partir de R$ 30
- Detalhamento de rodas — a partir de R$ 30
- Limpeza de caixas de roda — a partir de R$ 20
- Remoção de piche e resina — a partir de R$ 30
- Remoção de manchas localizadas — a partir de R$ 20
- Higienização interna localizada — a partir de R$ 30
- Limpeza detalhada do porta-malas — a partir de R$ 20
- Polimento de faróis — a partir de R$ 60
- Revitalização de borrachas externas — a partir de R$ 20
- Remoção de pelos — a partir de R$ 30
- Adicional por sujeira excessiva — a partir de R$ 20

### Novos adicionais para motos
- Lavagem técnica do motor — a partir de R$ 30
- Desengraxe pesado do motor — a partir de R$ 40
- Limpeza detalhada da relação — a partir de R$ 20
- Limpeza + lubrificação da corrente — a partir de R$ 25
- Restauração de escapamento — a partir de R$ 40
- Polimento de escapamento — a partir de R$ 30
- Revitalização de plásticos da moto — a partir de R$ 20
- Proteção / acabamento do motor — a partir de R$ 20
- Detalhamento de rodas — a partir de R$ 25
- Remoção de barro pesado — a partir de R$ 20
- Descontaminação de carenagens — a partir de R$ 30
- Hidrorepelência dos retrovisores — a partir de R$ 15
- Remoção de chuva ácida dos retrovisores — a partir de R$ 20
- Adicional por sujeira excessiva — a partir de R$ 20

A migração preserva o banco existente e o histórico de atendimentos. Adicionais antigos continuam preservados; alguns nomes genéricos são melhorados sem trocar seus IDs.

## V13.6 — Agenda semanal facilitada
- Configuração rápida de um horário padrão para vários dias.
- Atalhos Seg–Sex, Seg–Sáb e Todos.
- Edição da semana inteira com um único botão Salvar.
- Inputs de horário nativos, com seletor tipo despertador no celular.
- Calendário e atalhos para bloqueio manual de datas.
- Mantém V13.5: preços fixos/a partir de/sob avaliação, novos adicionais e avisos de avaliação.
- Mantém Configurações em abas, backup/restauração e Persistent Disk.

## V13.7 — Instagram e vínculo de cliente existente
- CTA e link para Instagram configurável pela Central de ajustes.
- O primeiro cadastro web reconhece cliente já cadastrado internamente pelo mesmo WhatsApp normalizado e vincula ao mesmo histórico/veículos.
- Aviso visual na Minha Garagem quando o vínculo automático acontece.

## V13.8 — Serviços e preços com abas dentro de abas

A tela Admin > Serviços e preços foi reorganizada para reduzir rolagem e mistura de categorias. Há duas abas principais (Preços e serviços / Serviços adicionais) e, dentro de cada uma, subabas Moto / Carro. A última seleção fica lembrada no navegador após salvar. Nenhuma regra financeira, de agendamento ou de banco foi alterada.

## V13.9 — Painel organizado em abas e subabas

A V13.9 aplica o padrão de navegação em abas às telas administrativas que tinham muitos blocos misturados. Configurações, agenda, perguntas do veículo, financeiro, detalhe do atendimento, galeria, promoções e avaliações agora usam abas/subabas apropriadas. Páginas simples permanecem simples para não criar cliques desnecessários. As abas internas lembram a última seleção no navegador.

## V14 — Nova Home Premium
- Cabeçalho público renovado com logo maior e wordmark mais legível.
- Menu hambúrguer responsivo no celular/tablet.
- Hero premium em 2 colunas no desktop e empilhado no mobile.
- Nova hierarquia visual preto/grafite/dourado/branco.
- Benefícios, CTAs e cards de métricas redesenhados.
- Serviços, galeria, Instagram, avaliações e CTA final com acabamento premium.
- Mantém integração com logo e imagens configuráveis pelo Admin.
- Sem alteração destrutiva de banco de dados.

## V15 — Home Premium, efeitos e controle de acesso
- Nova identidade padrão: `static/ph_logo_v15.png` e `static/ph_hero_v15.png`.
- Animações de entrada ao rolar com `IntersectionObserver`, respeitando `prefers-reduced-motion`.
- Logo da área de acesso/cadastro configurável separadamente.
- `admin_users`: mestre, gerente e funcionário com permissões por módulo.
- `Admin -> Meu acesso`: troca de usuário e senha.
- `Admin -> Usuários e acessos`: criação ilimitada de acessos e seleção simples de permissões.
- Usuário mestre é criado automaticamente a partir das credenciais administrativas antigas na primeira migração.

## V15.1 — Financeiro: editar e ver despesas
- Descrição e fornecedor agora aparecem em colunas separadas.
- A tabela de despesas usa: Data, Descrição, Fornecedor, Categoria, Competência, Valor, Status e Ações.
- Cada lançamento possui botões **Editar** e **Ver**, além das ações de pagamento/cancelamento já existentes.
- A tela **Ver** mostra forma de pagamento, vencimento, data de pagamento, observações e demais dados do lançamento.
- A tela **Editar** permite corrigir todos os campos principais sem excluir a despesa.
- Os cálculos e relatórios financeiros passam a considerar os dados corrigidos automaticamente.

## V15.2 — Modo TV premium + Área de testes
- `/tv` ganhou layout de tela grande, relógio, métricas, serviço/status e atualização automática.
- `/admin/testes` é um laboratório isolado para criar cenários fictícios usando os serviços e adicionais atuais.
- `/admin/testes/kanban` permite testar o fluxo operacional sem tocar nos agendamentos reais.
- `/tv/testes` mostra apenas os cenários fake da data atual.
- A Área de testes não movimenta clientes, agenda, receitas, despesas ou fechamento financeiro reais.
- Funcionários podem receber a nova permissão `Área de testes`; mestre e gerente têm acesso automaticamente.
